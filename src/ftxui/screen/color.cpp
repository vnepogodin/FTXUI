...
Color Color::HSV(uint8_t h, uint8_t s, uint8_t v) {
  ...
  const uint8_t remainder = (h - (region * 43)) * 6;                    // NOLINT
  const uint8_t p = (v * (255 - s)) >> 8;                               // NOLINT
  const uint8_t q = (v * (255 - ((s * remainder) >> 8))) >> 8;          // NOLINT
  const uint8_t t = (v * (255 - ((s * (255 - remainder)) >> 8))) >> 8;  // NOLINT
