...
static constexpr const std::array<Interval, 142> g_combining_characters = {
...
static constexpr const std::array<Interval, 13> g_full_width_characters = {
...
constexpr bool Bisearch(uint32_t ucs, const Interval* table, int max) {
...
constexpr bool IsCombining(uint32_t ucs) {
  return Bisearch(ucs, g_combining_characters.data(),
                  g_combining_characters.size() - 1);
}

constexpr bool IsFullWidth(uint32_t ucs) {
...
constexpr bool IsControl(uint32_t ucs) {
  if (ucs == 0) {
    return true;
  }
  if (ucs < 32) {  // NOLINT
...
constexpr int codepoint_width(uint32_t ucs) {
  if (IsControl(ucs)) {
    return -1;
  }
